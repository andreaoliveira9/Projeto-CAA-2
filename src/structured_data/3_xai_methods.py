...
def load_model_and_data():
    ...
    try:
        ...
        print(
            "Model or data files not found. Please run 1_data_preprocessing.py and 2_model_training.py first."
        )
...
def apply_shap_simple(model, X_train, X_test, feature_names):
    ...
    try:
        ...
        feature_importance.to_csv(
            "output/shap_results/shap_feature_importance.csv", index=False
        )
        ...
        plt.barh(
            ...
            feature_importance.iloc[sorted_idx]["SHAP_Importance"],
        )
        plt.yticks(
            range(len(sorted_idx)),
            [feature_importance.iloc[i]["Feature"] for i in sorted_idx],
        )
        ...
        for i in range(min(5, len(X_sample))):
            ...
            instance_importance = [
                (feature_names[j], abs(instance_shap[j]))
            ...
            shap_values_top = [
                instance_shap[feature_names.index(f[0])] for f in top_features
            ]
...
def apply_permutation_importance(model, X_test, y_test, feature_names):
    ...
    perm_importance_df.to_csv(
        "output/xai_results/permutation_importance.csv", index=False
    )
    ...
    end_time = time.time()
    print(
        f"Permutation importance analysis completed in {end_time - start_time:.2f} seconds."
    )
    print(f"Top 5 important features:")
    for i in range(5):
        feature = perm_importance_df.iloc[i]
        print(
            f"{i+1}. {feature['Feature']}: {feature['Importance']:.4f} ± {feature['Std']:.4f}"
        )

    return perm_importance_df


def apply_lime_values(model, X_train, X_test, feature_names):
    """Apply LIME to explain model predictions and output feature importances."""
    print("\nApplying LIME analysis...")
    start_time = time.time()
    ensure_directory_exists("output/lime_results")

    # Sample a small subset for LIME analysis
    sample_size = min(50, len(X_test))
    X_sample = X_test.sample(sample_size, random_state=42)
    ...
    feature_importance = pd.DataFrame(
        0, index=feature_names, columns=["LIME_Importance"]
    )

    # Explain each instance and accumulate importances
    for i, (_, row) in enumerate(X_sample.iterrows()):
        ...
        for feature, importance in exp.as_list():
            # feature is like 'feature_name <= value' or 'feature_name > value', so extract feature_name
            feature_name = feature.split(" ")[0]
            if feature_name in feature_importance.index:
                feature_importance.loc[feature_name, "LIME_Importance"] += abs(
                    importance
                )
        # Salvar plot individual para as primeiras 5 instâncias
        if i < 5:
            fig = exp.as_pyplot_figure()
            plt.tight_layout()
            plt.savefig(f"output/lime_results/instance_{i+1}_lime_explanation.png")
            plt.close(fig)

    # Average importance over all samples
    feature_importance["LIME_Importance"] /= sample_size
    feature_importance = feature_importance.sort_values(
        "LIME_Importance", ascending=False
    )

    # Save to CSV
    feature_importance.to_csv("output/lime_results/lime_feature_importance.csv")

    # Plot feature importance
    plt.figure(figsize=(10, 8))
    plt.barh(feature_importance.index, feature_importance["LIME_Importance"])
    plt.xlabel("Mean |LIME Value|")
    plt.title("Feature Importance Based on LIME Values")
    plt.tight_layout()
    plt.savefig("output/lime_results/lime_feature_importance.png")
    plt.close()

    print("\nTop 5 most important features according to LIME:")
    for i, (feature, row) in enumerate(feature_importance.head(5).iterrows()):
        print(f"{i+1}. {feature}: {row['LIME_Importance']:.4f}")

    end_time = time.time()
    print(f"LIME analysis completed in {end_time - start_time:.2f} seconds.")
    return feature_importance
...
def compare_xai_methods(shap_df, perm_df):
    ...
    comparison = pd.DataFrame(
        index=all_features, columns=["SHAP_rank", "Permutation_rank"]
    )
    ...
    plt.figure(figsize=(12, 8))
    chart = sns.barplot(
        x="Feature", y="Importance", hue="Method", data=comparison_melted
    )
...
def main():
    ...
    # Apply LIME analysis
    lime_importance = apply_lime_values(model, X_train, X_test, feature_names)
    ...
    print("\nLIME results saved in the 'output/lime_results' directory.")

    return shap_importance, perm_importance, lime_importance, comparison
